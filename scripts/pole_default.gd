...
#TODO: вынести сохранение в отдельный файл

func quicksave():
	save_to_file("res://quicksave.tres", save_to_res())

func quickload():
	load_from_res(load_from_file("res://quicksave.tres"))
...
func save_to_res() -> pole_save:
	...
	var cells = get_used_cells() #таблица векторов с занятыми клетками
	...
	for one_cell in cells:
		cell_types[one_cell] = get_cell_type(one_cell)
...
func load_from_res(res: pole_save):
	...
	for child in res.ball_emitters:
		...
		call_deferred("add_child", my_emitter)
